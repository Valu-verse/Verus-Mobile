 ...
 */
export const validateAppEncryptionRequestVDXFObject = (request, detailIndex) => {
  const detailsObject = request.getDetails(detailIndex);

  if (!(detailsObject instanceof AuthenticationRequestOrdinalVDXFObject)) {
    throw new Error("Encryption request details not found at specified index");
  }

  if (detailsObject.data == null || !detailsObject.data.isValid()) {
    throw new Error("Invalid encryption request details.");
  }
}
